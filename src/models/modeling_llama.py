...
from common import RangeWeight
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        range_weights: Optional[List[RangeWeight]] = None,
        ...
        **kwargs,
        ...
        if (
            ...
            original_sum = torch.sum(
                torch.stack(
                    [
                        torch.sum(attn_weights[:, :, :, rw.start : rw.end], dim=-1)
                        for rw in range_weights
                    ]
                ),
                dim=0,
            )
            ...
            for rw in range_weights:
                attn_weights[:, :, :, rw.start : rw.end] *= rw.weight
            ...
            norm_factor = (original_sum / new_sum).unsqueeze(-1)
            for rw in range_weights:
                attn_weights[:, :, :, rw.start : rw.end] *= norm_factor
        ##### </AttentionScaling> #####
    ...
    def _update_causal_mask(self, attention_mask, input_tensor):
        ...
        if self.config._attn_implementation == "sdpa":
            ...
            if (
                ...
                and torch.any(attention_mask != 1)
            ):
